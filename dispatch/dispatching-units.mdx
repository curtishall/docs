...
If this is the **last field unit** on the call, Cadmus asks you to confirm **Return call to Pending**. Confirming unassigns the unit, sets the unit Available, and moves the CFS back to the **Pending** queue so another unit can be dispatched. Agency desk units left on the call do not count as a field assignment.

Field MDT users cannot orphan a call this way. A deputy who is the last unit must **close** the call instead of unassigning.

<Warning>
  Returning a high-priority in-progress call to Pending should be coordinated with a supervisor. The call is no longer assigned to a field unit until someone is dispatched again.
</Warning>
