...
### Returning the call to Pending (pre-empt)

When you clear the **last field unit** from a call as a dispatcher, Cadmus can return the call to **Pending** instead of leaving it active with no units. Confirm **Return to Pending** when prompted. The call reappears on the Pending board so any unit can be assigned. See [Pre-empt / Return to Pending](/dispatch/dashboard-overview#pre-empt--return-to-pending) for CFS behavior when the wrong agency was assigned.
