...
| `STATUS OFF @Agency` | Pause the **agency** status timer for that agency on this call (same as **Pause checks** on the timer card). Use agency **ID** or **name** after `@` when multiple agencies are attached. |
| `STATUS ON @Agency` | Resume that agency's status timer and restart the check interval (same as **Resume checks** on the timer card). |
